...
try {
    ...
    if ($Batt) {
        ...
        switch ($Batt.BatteryStatus) {
            1 { Write-Host "State: Discharging" -ForegroundColor Yellow }
            2 { Write-Host "State: AC Connected" -ForegroundColor Green }
            3 { Write-Host "State: Fully Charged" -ForegroundColor Green }
            4 { Write-Host "State: Low Battery" -ForegroundColor Red; $BatteryScore -= 10 }
            5 { Write-Host "State: Critical Battery" -ForegroundColor Red; $BatteryScore -= 20 }
        }
    }
} catch {
...
# ---------------------------------------------------------------------
# 2. DETAILED CAPACITY & WEAR LEVEL (via battery report)
# ---------------------------------------------------------------------
...
if (Test-Path $TempReport) {
    ...
    if ($DesignCap -and $FullChargeCap) {
        $Wear = [math]::Round((1 - ($FullChargeCap / $DesignCap)) * 100, 2)
        ...
        Write-Host "Battery Wear Level  : $Wear%" -ForegroundColor `
            $(if ($Wear -lt 10) {"Green"} elseif ($Wear -lt 20) {"Yellow"} else {"Red"})
        ...
        Write-Host "Unable to extract capacity info from battery report." -ForegroundColor Gray
    }
}

# ---------------------------------------------------------------------
# 3. CYCLE COUNT (if ACPI OEM exposes)
# ---------------------------------------------------------------------
...
# ---------------------------------------------------------------------
# 5. BATTERY TEMPERATURE (if available)
# ---------------------------------------------------------------------
...
try {
    ...
    foreach ($T in $Thermal) {
        ...
        if ($T.InstanceName -match "BAT" -or $T.InstanceName -match "Battery") {
            $Temps += $C
            Write-Host "Battery Temperature: $([math]::Round($C,1))°C" -ForegroundColor `
                $(if ($C -gt 50) {"Red"} elseif ($C -gt 40) {"Yellow"} else {"Green"})
    ...
    if ($Temps.Count -eq 0) {
        Write-Host "Battery temperature not exposed by firmware." -ForegroundColor Gray
    }
} catch {
...
# ---------------------------------------------------------------------
# 6. BATTERY HEALTH SCORE & FINAL RECOMMENDATIONS
# ---------------------------------------------------------------------
...
Write-Host "`nFinal Battery Score: $BatteryScore/100" -ForegroundColor `
    $(if ($BatteryScore -ge 80) {"Green"} elseif ($BatteryScore -ge 60) {"Yellow"} else {"Red"})

if ($BattIssues) {
    Write-Host "`n CRITICAL ISSUES:" -ForegroundColor Red
    $BattIssues | ForEach-Object { Write-Host "  • $_" -ForegroundColor Red }
}

if ($BattWarnings) {
    Write-Host "`n WARNINGS:" -ForegroundColor Yellow
    $BattWarnings | ForEach-Object { Write-Host "  • $_" -ForegroundColor Yellow }
}
...
# Convert to JSON and output it as a pure string so Python can easily capture it
