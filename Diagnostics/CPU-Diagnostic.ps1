...
Write-Host "===================================================="
Write-Host " CPU DETAILED DIAGNOSTIC"
Write-Host "====================================================`n"

# ======================================================================
# 1. CPU INFORMATION
# ======================================================================
Write-Host "[1] CPU INFORMATION" -ForegroundColor Yellow
...
1..10 | ForEach-Object {
    ...
    Write-Host "Sample $_ : $([math]::Round($Usage, 1))%" `
        -ForegroundColor $(
            if ($Usage -gt 80) {
                "Red"
            } elseif ($Usage -gt 50) {
...
# ======================================================================
Write-Host "`n[3] PER-CORE CPU USAGE" -ForegroundColor Yellow

$CoreUsage = Get-Counter `
    "\Processor(*)\% Processor Time" `
    -SampleInterval 2 |
    Where-Object {
        $_.CounterSamples.InstanceName -notlike "*_total*"
    } |
...
$CoreUsage |
    Format-Table InstanceName, Usage -AutoSize
...
# ======================================================================
# 5. CPU TEMPERATURE AND HEALTH
# ======================================================================
Write-Host "`n[5] CPU TEMPERATURE AND HEALTH" -ForegroundColor Yellow

try {
    $Temp = Get-CimInstance `
        -Namespace root\WMI `
    ...
    if ($Temp) {
        ...
        Write-Host "Current Temperature: $([math]::Round($CurrentTemp, 1))°C" `
            -ForegroundColor $(
...
$AvgUsage = (
    $CoreUsage |
    Measure-Object -Property Usage -Average
).Average
...
# Include only the top five CPU-consuming processes in the JSON payload
# to keep downstream LLM analysis compact.
...
$DiagnosticResult = [ordered]@{
    ...
    "AvgUsagePct"   = if ($null -ne $AvgUsage) {
        [math]::Round($AvgUsage, 2)
    } else {
        0
    }
    "TemperatureC"  = if ($null -ne $CurrentTemp) {
        [math]::Round($CurrentTemp, 1)
    } else {
        $null
    }
    "TopProcesses"  = $Top5Processes
    "Status"        = if ($AvgUsage -gt 80) {
        "High"
    } elseif ($AvgUsage -gt 50) {
        "Moderate"
    } else {
