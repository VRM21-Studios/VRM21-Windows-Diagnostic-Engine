...
# Purpose : Analyze installed memory, current usage, memory composition,
#           high-memory processes, and basic memory-related conditions.
# ======================================================================
...
Write-Host "Used: $UsedMemory GB ($UsagePercent%)" -ForegroundColor $(
...
if ($CacheMemory) {
    $CacheMB = [math]::Round(
        $CacheMemory.CounterSamples.CookedValue / 1MB,
        2
    )
...
# ----------------------------------------------------------------------
# 5. High-Memory Process Detection
# ----------------------------------------------------------------------
...
# Collect the five highest-memory processes to limit JSON output size
# and reduce unnecessary context for downstream analysis.
$Top5MemoryProcs = @()
...
$DiagnosticResult = [ordered]@{
    ...
    "PageFileGB"          = if ($null -ne $PageFile) {
        [math]::Round($PageFile.AllocatedBaseSize / 1GB, 2)
    } else {
        0
    }
    "CacheMB"             = if ($null -ne $CacheMB) { $CacheMB } else { 0 }
    "HighMemProcessCount" = if ($null -ne $HighMemoryProcesses) {
        @($HighMemoryProcesses).Count
    } else {
        0
    }
    "TopProcesses"        = $Top5MemoryProcs
    "Status"              = if ($UsagePercent -gt 90) {
        "Critical"
    }
    elseif ($UsagePercent -gt 80) {
        "Warning"
