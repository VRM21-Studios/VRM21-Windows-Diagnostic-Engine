...
if (-not $GPUs) {
    ...
    foreach ($GPU in $GPUs) {
        ...
        # Check if integrated or dedicated
    ...
    if ($GPUUsage) {
        ...
        if ($ActiveGPUs) {
            ...
            foreach ($GPU in $ActiveGPUs) {
                ...
                Write-Host "  $($GPU.Name): $([math]::Round($MaxUsage, 1))% usage" -ForegroundColor $(if($MaxUsage -gt 80){"Red"}elseif($MaxUsage -gt 50){"Yellow"}else{"Green"})
            }
        } else {
...
if ($GraphicsProcesses) {
    ...
    $GraphicsProcesses | Format-Table ProcessName, Id,
...
# 6. GPU Temperature (if available via WMI)
Write-Host "`n[6] GPU TEMPERATURE & HEALTH" -ForegroundColor Yellow
try {
    # Try to get GPU temperature from various WMI namespaces
    $TempSources = @(
        "\\.\ROOT\CIMV2", 
        "\\.\ROOT\WMI", 
    ...
    foreach ($Namespace in $TempSources) {
        try {
            $Temperatures = Get-CimInstance -Namespace $Namespace -ClassName *temperature* -ErrorAction SilentlyContinue
            if ($Temperatures) {
                foreach ($Temp in $Temperatures) {
                    if ($Temp.Name -match "GPU" -or $Temp.Description -match "GPU") {
                        $CurrentTemp = if ($Temp.CurrentTemperature) { $Temp.CurrentTemperature } 
                                      elseif ($Temp.Temperature) { $Temp.Temperature }
                                      else { $null }
                        
                        if ($CurrentTemp) {
                            # Convert to Celsius if needed
                            $TempC = if ($CurrentTemp -gt 100) { $CurrentTemp } else { $CurrentTemp }
                            Write-Host "GPU Temperature: $([math]::Round($TempC, 1))°C" -ForegroundColor $(if($TempC -gt 85){"Red"}elseif($TempC -gt 75){"Yellow"}else{"Green"})
                            $TempFound = $true
                            break
                        }
                    }
...
$DedicatedGPUs = $GPUs | Where-Object { $_.Name -notmatch "Intel|HD Graphics|UHD Graphics" }
$IntegratedGPUs = $GPUs | Where-Object { $_.Name -match "Intel|HD Graphics|UHD Graphics" }
...
if ($IntegratedGPUs -and $DedicatedGPUs) {
    Write-Host " Hybrid Graphics System:" -ForegroundColor Cyan
    Write-Host "   - Use graphics settings to assign apps to dedicated GPU" -ForegroundColor White
...
if ($IntegratedGPUs -and -not $DedicatedGPUs) {
    ...
    Write-Host "   - Close unnecessary background apps for gaming" -ForegroundColor White
}
...
foreach ($GPU in $GPUs) {
    if ($GPU.AdapterRAM -lt 2GB) {
        ...
        Write-Host "   - Consider lowering texture quality in games" -ForegroundColor White
    }
}
...
if ($GPUs) {
    foreach ($G in $GPUs) {
        $GPUList += [ordered]@{
            ...
            "IsDedicated" = if ($G.Name -match "Intel|HD Graphics|UHD Graphics|Radeon Graphics") { $false } else { $true }
        }
    }
}
...
$GraphicsProcs = @()
if ($GraphicsProcesses) {
    # Ambil top 5 proses saja agar context window LLM tidak terlalu penuh
